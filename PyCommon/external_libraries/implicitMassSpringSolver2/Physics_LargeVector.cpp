...
// ys
bool Physics_LargeVector::ElementPostMultiply( Physics_Matrix3x3 S[], Physics_LargeVector &dst )
{
	if( m_iElements != dst.m_iElements )
		return false;
	...
	for( int i=0; i<m_iElements; i++ )
	{
		S[i].PostMultiply( m_pData[i], dst.m_pData[i] );
	}
	return true;
}

bool Physics_LargeVector::Scale( Physics_t scale, Physics_LargeVector &dst )
{
	if( m_iElements != dst.m_iElements )
		return false;
...
bool Physics_LargeVector::Invert( Physics_LargeVector &dst )
{
	...
	for( int i=0; i<m_iElements; i++ )
	{
		dst.m_pData[i].x = (Physics_t)1.0 / m_pData[i].x;
		dst.m_pData[i].y = (Physics_t)1.0 / m_pData[i].y;
		dst.m_pData[i].z = (Physics_t)1.0 / m_pData[i].z;
	}
	return true;
}

Physics_t Physics_LargeVector::DotProduct( Physics_LargeVector &v )
{
	Physics_t res = (Physics_t)0.0;
