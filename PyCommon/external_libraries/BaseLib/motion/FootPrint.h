...
namespace MotionUtil
{
	...
	/// GetFootPrint uses the pre-stored foot position information in the Poses.
	// if footprints are too far from constraints, turn it off.
	class GetFootPrintOnlineHeu : public FootPrint
	{
		interval mHeightInterval;
		m_real mLengthThr;
		m_real mDistThr;
	public:
		GetFootPrintOnlineHeu(m_real minHeight=0.0, m_real maxHeight=4.0, m_real lengthThr=0.95, m_real distThr=0.3);
		virtual ~GetFootPrintOnlineHeu(){}
		virtual void getFootPrints(const Motion& mot, int start, int end, int constraint, 
									intvectorn& interval, matrixn& aFootPositions) const;		
	};
