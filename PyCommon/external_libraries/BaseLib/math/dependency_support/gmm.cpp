...
{
	std::swap(inout, gmm::wsmatrixn (n, m));
}
...
void SparseQuadraticFunctionHardCon ::buildSystem(gmm::wsmatrixn & Augmented, vectorn &d)
{
	...
	SparseQuadraticFunction::buildSystem(nvar, Augmented, d.range(0, nvar));
...
// ys
void SparseQuadraticFunctionHardCon::clear()
{
	clearSquaredTerms();
	clearConstraints();
}
// ys
void SparseQuadraticFunctionHardCon::clearSquaredTerms()
{
	QuadraticFunction::clear();
}
// ys
void SparseQuadraticFunctionHardCon::clearConstraints()
{
	for(std::list<Con*>::iterator i=mListConTerms.begin(); i!=mListConTerms.end(); ++i)
		delete (*i);
	mListConTerms.clear();
