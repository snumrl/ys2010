...
// ys
void QuadraticFunction::addSquaredWeighted(m_real weight, intvectorn const& index, vectorn const& value)
{
	SquaredTerm* term=new SquaredTerm;
	mListSQTerms.push_back(term);
...
void QuadraticFunctionHardCon ::buildSystem(matrixn & Augmented, vectorn &d)
{
	...
	QuadraticFunction::buildSystem(nvar, H, d.range(0, nvar));
...
m2::c1stitchPreprocess::c1stitchPreprocess(int arow, int brow, m_real strength,bool bConMid)
:mArow(arow), mBrow(brow), h(arow+brow-1), mbConMid(bConMid), mStrength(strength)
{
	...
	invAugmented.op1(m1::LUinvert(), Augmented);
}	

void m2::c1stitchPreprocess::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
...
void m0::c0stitch::calc(matrixn& c) const 
{
//#define DEBUG_C0STITCH
	...
	center.op2(v2::interpolate(t), c.row(discontinuity-1), c.row(discontinuity));
	...
	for(int i=0; i<discontinuity; i++)
		c.row(i).op1(v1::radd(), strans*pow(double(i)/ ((double)discontinuity-0.5),mStrength));

	for(int i=discontinuity; i<c.rows(); i++)
		c.row(i).op1(v1::radd(), atrans*pow(1.f-(double(i-discontinuity)+0.5)/ ((double)(c.rows()-discontinuity)-0.5), mStrength));
#ifdef DEBUG_C0STITCH
	c.op0(m0::drawSignals(sz1::format0("%4d.bmp", currS++), 0, 0, true));
#endif
...
m2::linstitchPreprocessInc::linstitchPreprocessInc(int arow, int brow, int nIter, int normalize,m_real strength)
...
void m2::linstitchPreprocessInc::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	Msg::verify(a.rows()==mArow , "error! A row different %d %d", a.rows(), mArow);
	Msg::verify(b.rows()==mBrow, "error! B row different %d %d", b.rows(), mBrow);
	int nsample=a.rows()+b.rows()-1;
...
m2::linstitchPreprocess::linstitchPreprocess(int arow, int brow, m_real strength, bool bConMid)
...
void m2::linstitchPreprocess::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	Msg::verify(a.rows()==mArow , "error! A row different %d %d", a.rows(), mArow);
	Msg::verify(b.rows()==mBrow, "error! B row different %d %d", b.rows(), mBrow);
	int nsample=a.rows()+b.rows()-1;
...
void m2::c0concat::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	// I used cos function not to distort the start and end position.
	if(a.rows()<2)
	{
	...
	vectorn center;
	center.each2(s2::AVG, a.row(a.rows()-1), b.row(0));
...
void m2::linstitch::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	/* LAGRANGIAN MULTIPLIER version*/
	//    Augmented* x= d , w=(X, lamda)
	//   (H A^T) (X    )= (d)
...
void m2::c0stitch::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	// I used cos function not to distort the start and end position.
	if(a.rows()<2)
	{
...
void m2::c0stitchPreserve2::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	// I used cos function not to distort the start and end position.
	...
	m2::c0stitch t;
	t.calc(c.range(1, c.rows()-1), a.range(1, a.rows()), b.range(0, b.rows()-1));
}

void m2::c1stitch::calc(matrixn& c, const matrixn& a, const matrixn& b) const
{
	m2::c1stitchPreprocess p(a.rows(), b.rows(), mStrength);
	p.calc(c, a, b);
}
...
namespace m2
{
	void m2::stitchQuaterNN ::calc(matrixn& c, const matrixn& a, const matrixn&b )const	
	{
		c.setSize(a.rows()+b.rows()-1, a.cols());
		ASSERT(a.cols()==b.cols());
		ASSERT(a.cols()%4==0);
		...
		else
		{
			...
			m2::stitchQuaterNN t(mFunc);
			t.calc(c.range(mPreserveAmount, c.rows()-mPreserveAmount), 
				a.range(mPreserveAmount, a.rows()),
				b.range(0, b.rows()-mPreserveAmount));
		}
	}

	void m2::linstitchForward::calc(matrixn& ddd, const matrixn& a, const matrixn& b) const
	{
		/* LAGRANGIAN MULTIPLIER version*/
		//    Augmented* x= d , w=(X, lamda)
		//   (H A^T) (X    )= (d)
...
#define fb(xxx)	b(xxx,dim)
	...
	void linstitchOnline::calc(matrixn& ddd, const matrixn& a, const matrixn& b) const
	{
		/* LAGRANGIAN MULTIPLIER version*/
		//    Augmented* x= d , w=(X, lamda)
		//   (H A^T) (X    )= (d)
...
#define fb(xxx)	b(xxx,dim)
	...
	void linstitch2::calc(matrixn& c, const matrixn& a, const matrixn& b) const
	{
		/* LAGRANGIAN MULTIPLIER version*/
		//    Augmented* x= d , w=(X, lamda)
		//   (H A^T) (X    )= (d)
...
#define fb(xxx)	b(xxx,dim)
	...
	void m2::linstitchMulti::calc(matrixn& c, const matrixn& a, const matrixn& b) const
	{
		/* LAGRANGIAN MULTIPLIER version*/
		//    Augmented* x= d , w=(X, lamda)
		//   (H A^T) (X    )= (d)
