...
namespace v1
{
	...
	struct projection : public _op
	{
		projection(const intervalN& iv):m_iv(iv){}
		virtual void calc(vectorn& c, const vectorn& a) const { m_iv.project(a,c);}
		const intervalN& m_iv;
	};
