...
matrixn&  matrixn::distanceMat(matrixn const& a, Metric* pMetric)
{
	...
	for(int i=0; i<a.rows(); i++)
	{
		...
		{
			(*this)[j][i]=(*this)[i][j]=a.row(i).distance(a.row(j), pMetric);
		}
	}
	return *this;
}

matrixn&  matrixn::distance(matrixn const& a, matrixn const& b, Metric* pMetric)
{
	...
	for(int i=0; i<a.rows(); i++)
	{
		...
		{
			(*this)[i][j]=a.row(i).distance(b.row(j),pMetric);
		}
	}
	return *this;
}
