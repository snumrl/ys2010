...
class vector2NView;
class vector2N : public _tvectorn<vector2, m_real, 2>
{	
protected:
	vector2N (m_real* ptrr, int size, int stride):_tvectorn<vector2, m_real, 2>(ptrr,size,stride){}
public:
	vector2N ():_tvectorn<vector2, m_real, 2>(){}

	// 값을 카피해서 받아온다.	
	template <class VecType>
	vector2N (const VecType& other)	{ assign(other);}

	explicit vector2N ( int x):_tvectorn<vector2, m_real, 2>() { setSize(x);}
	~vector2N(){}
	...
	template <class VecType>
	void operator=(const VecType& other)	{ _tvectorn<vector2, m_real, 2>::assign(other);}
