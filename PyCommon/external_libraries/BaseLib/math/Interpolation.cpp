...
#include "../baselib/math/statistics.h"
...
void InterpolationNormalize::Test(const vectorn& signal1D, const char* testID, int TestResolution)
{
	...
	{
		cip.DrawVertLine(parameters.argNearest(signal1D[i]), 0, cip.Height(), RGB(0,0,0),true);
	}
...
KNearestInterpolation ::KNearestInterpolation (Metric* pMetric, int k, float power)
{
	m_pMetric=pMetric; m_nK=k; m_fK=power;
}
...
void LCBInterpolation::initialize(const matrixn& sourceSamples)
{
	...
	// I will completely reimplement this shit!
}
...
*/
	...
	matrixn distMat;
	distMat.distanceMat(m_aNormalizedSamples);
...
void RBInterpolation::initialize(const matrixn& sourceSamples)
{
	...
	// heuristic : 2*min distance
	matrixn distMat;
	distMat.distance(m_aNormalizedSamples);
	distMat.setDiagonal(FLT_MAX);
	...
	else
	{
		...
		// heuristic : 2*min distance
		matrixn sortedDistMat;
		sortedDistMat.each1(v1::sort(), distMat);
		sortedDistMat.resize(sortedDistMat.rows(), MIN(sortedDistMat.cols()-1,3));
		m_aDilationFactor.aggregate(CAggregate::AVG, sortedDistMat);
		m_aDilationFactor*=2;
/*
		// heuristic : max(min(distMat));
		vectorn min;
