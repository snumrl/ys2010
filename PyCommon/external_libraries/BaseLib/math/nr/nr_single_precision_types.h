#ifndef _NR_single_precision_TYPES_H_
#define _NR_single_precision_TYPES_H_
...
typedef const vectornSP Vec_I_DP;
typedef vectornSP Vec_DP;
typedef vectornSP Vec_O_DP, Vec_IO_DP;

typedef const NRVec<complex<float> > Vec_I_CPLX_SP;
typedef NRVec<complex<float> > Vec_CPLX_SP, Vec_O_CPLX_SP, Vec_IO_CPLX_SP;

typedef const NRVec<complex<NR_single_precision::DP> > Vec_I_CPLX_DP;
typedef NRVec<complex<NR_single_precision::DP> > Vec_CPLX_DP, Vec_O_CPLX_DP, Vec_IO_CPLX_DP;
...
typedef const matrixnSP Mat_I_DP;
typedef matrixnSP Mat_DP;
typedef matrixnSP Mat_O_DP, Mat_IO_DP;


typedef const NRMat<complex<float> > Mat_I_CPLX_SP;
typedef NRMat<complex<float> > Mat_CPLX_SP, Mat_O_CPLX_SP, Mat_IO_CPLX_SP;

typedef const NRMat<complex<NR_single_precision::DP> > Mat_I_CPLX_DP;
typedef NRMat<complex<NR_single_precision::DP> > Mat_CPLX_DP, Mat_O_CPLX_DP, Mat_IO_CPLX_DP;

// 3D Matrix Types

typedef const NRMat3d<NR_single_precision::DP> Mat3D_I_DP;
typedef NRMat3d<NR_single_precision::DP> Mat3D_DP, Mat3D_O_DP, Mat3D_IO_DP;

// Miscellaneous Types

typedef NRVec<unsigned long *> Vec_ULNG_p;
typedef NRVec<NRMat<NR_single_precision::DP> *> Vec_Mat_DP_p;
typedef NRVec<fstream *> Vec_FSTREAM_p;
}
#endif /* _NR_single_precision_TYPES_H_ */
