...
void bitvectorn::cluster(int windowSize)
{
	matrixn aInputVec;
	int numTrue=count();
	aInputVec.setSize(numTrue, 1);
	int trueIndex=0;
	for(int i=0; i<numTrue; i++)
	{
		trueIndex=find(trueIndex, true);
		ASSERT(trueIndex!=size());
		aInputVec[i][0]=(m_real)trueIndex;
		trueIndex++;
	}
	ASSERT(find(trueIndex, true)==size());

	int GroupNum;
	intvectorn GroupIndex;
	GroupIndex.setSize(numTrue);
	matrixn aCenter;
	CFuzzyCluster::GenerateOptimumCluster(aInputVec, GroupNum, GroupIndex.dataPtr(), windowSize, aCenter);

	clearAll();
	for(int i=0; i<aCenter.rows(); i++)
	{
		setAt(int(aCenter[i][0]+0.5));
	}
}
