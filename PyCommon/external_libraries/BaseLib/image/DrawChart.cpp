...
DrawChart::DrawChart(const char* xaxis, const char* yaxis, m_real minx, m_real maxx, m_real miny, m_real maxy)
{
	...
	mYaxis=yaxis;
	mInterval.setSize(2);
	mInterval.start_ref().setValues(2, minx, miny);
	mInterval.end_ref().setValues(2, maxx, maxy);
	mpImage=new CImage();
	mpImage->Create(SIZE, SIZE, 24);
	mpCanvas.Init(mpImage);
	mpCanvas.Clear(RGB(255,255,255));
}
...
void DrawChart::drawMatrix(matrixn const& matrix)
{
	...
	for(int i=0; i<matrix.rows(); i++)
		for(int j=0; j<matrix.cols(); j++)
		{
			...
			mpCanvas.DrawBox(TRect(cx[i]-width, cy[j]-height, cx[i]+width, cy[j]+height), RGB(color, color, color));
		}

}


void DrawChart::drawScatteredData(matrixn const& matrix, COLORREF color, char* patternFile)
{
	CImage pattern;
	int dist;
	if(patternFile)
	...
	if(patternFile)
	{
		if(color==RGB(255,255,255))
			for(int i=0; i<matrix.rows(); i++)
			{
				...
				mpCanvas.DrawPattern(x-dist, y-dist, &pattern, true, RGB(0,0,0));
			}
		else
			for(int i=0; i<matrix.rows(); i++)
			{
				int x=canvasX(matrix[i][0]);
				int y=canvasY(matrix[i][1]);
				mpCanvas.DrawPattern(x-dist, y-dist, &pattern, true, RGB(0,0,0), true, color);
			}
	}
	else
	{
